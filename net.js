...
NetPanel.prototype = domplate(Firebug.ActivablePanel,
{
    ...
    fileTag:
        FOR("file", "$files",
            TR({class: "netRow $file.file|getCategory",
                ...
                TD({class: "netSizeCol netCol"},
                    DIV({class: "netSizeLabel netLabel"}, "$file.file|getAppEngineCost")
                ),
                TD({class: "netSizeCol netCol"},
                    DIV({class: "netSizeLabel netLabel"}, "$file.file|getAppEngineTime")
                ),
                TD({class: "netSizeCol netCol"},
                    DIV({class: "netSizeLabel netLabel"}, "$file.file|getAppEngineCPUTime")
                ),
                TD({class: "netSizeCol netCol"},
                    DIV({class: "netSizeLabel netLabel"}, "$file.file|getAppEngineAPITime")
                ),
    ...
    activationTag:
        TR({class: "netRow netActivationRow"},
            TD({class: "netCol netActivationLabel", colspan: 9},
    ...
    summaryTag:
        TR({class: "netRow netSummaryRow"},
            ...
            TD({class: "netTotalSizeCol2 netCol"},
                DIV({class: "netTotalSizeLabel2 netSummaryLabel2"}, "")
            ),
            TD({class: "netTotalSizeCol3 netCol"},
                DIV({class: "netTotalSizeLabel3 netSummaryLabel3"}, "")
            ),
            TD({class: "netTotalSizeCol4 netCol"},
                DIV({class: "netTotalSizeLabel4 netSummaryLabel4"}, "")
            ),
            TD({class: "netTotalSizeCol5 netCol"},
                DIV({class: "netTotalSizeLabel5 netSummaryLabel5"}, "")
            ),
    ...
    summarizePhase: function(phase, rightNow)
    {
        ...
        var appEngineCost = 0, appEngineTime = 0, appEngineCPUTime = 0, appEngineAPITime = 0;
        ...
        for (var i=0; i<phase.files.length; i++)
        {
            ...
            if (!category || file.category == category)
            {
                if (file.loaded)
                {
                    ...
                    appEngineCost += file.appEngineCost;
                    appEngineTime += file.appEngineTime;
                    appEngineCPUTime += file.appEngineCPUTime;
                    appEngineAPITime += file.appEngineAPITime;
                }
            }
        }
        ...
        return {cachedSize: cachedSize, totalSize: totalSize, totalTime: totalTime,
                appEngineCost : appEngineCost, appEngineTime : appEngineTime, 
                appEngineCPUTime : appEngineCPUTime, appEngineAPITime : appEngineAPITime,
                fileCount: fileCount}
    },
    ...
    formatAppEngineCost: function(amount)
    {
        if (amount == 0) return "";
        return ("$" + amount + "000000000").substr(0,9);
    },

    formatAppTime: function(elapsed)
    {
        if (elapsed == 0) return "";
        return this.formatTime(elapsed);
    },

    getAppEngineCost: function(file)
    {
        return this.formatAppEngineCost(file.appEngineCost);
    },

    getAppEngineTime: function(file)
    {
        return this.formatAppTime(file.appEngineTime);
    },

    getAppEngineCPUTime: function(file)
    {
        return this.formatAppTime(file.appEngineCPUTime);
    },

    getAppEngineAPITime: function(file)
    {
        return this.formatAppTime(file.appEngineAPITime);
    },
    ...
    updateFileRow: function(file, newFileData)
    {
        ...
        else
        {
            ...
            var label = row.childNodes[4].firstChild;
            label.innerHTML = this.getAppEngineCost(file);

            label = row.childNodes[5].firstChild;
            label.innerHTML = this.getAppEngineTime(file);

            label = row.childNodes[6].firstChild;
            label.innerHTML = this.getAppEngineCPUTime(file);

            label = row.childNodes[7].firstChild;
            label.innerHTML = this.getAppEngineAPITime(file);
            ...
            var timeLabel = row.childNodes[8].firstChild.lastChild.firstChild;
    ...
    updateTimeline: function(rightNow)
    {
        ...
        for (var row = tbody.firstChild; row; row = row.nextSibling)
        {
            ...
            // Get bar nodes
            var resolvingBar = row.childNodes[8].firstChild.childNodes[1];
    ...
    updateSummaries: function(rightNow, updateAll)
    {
        ...
        for (var i = 0; i < phases.length; ++i)
        {
            ...
            appEngineCost += summary.appEngineCost;
            appEngineTime += summary.appEngineTime;
            appEngineCPUTime += summary.appEngineCPUTime;
            appEngineAPITime += summary.appEngineAPITime;
        }
        ...
        var label = row.childNodes[4].firstChild;
        label.innerHTML = this.formatAppEngineCost(appEngineCost);

        label = row.childNodes[5].firstChild;
        label.innerHTML = this.formatAppTime(appEngineTime);

        label = row.childNodes[6].firstChild;
        label.innerHTML = this.formatAppTime(appEngineCPUTime);

        label = row.childNodes[7].firstChild;
        label.innerHTML = this.formatAppTime(appEngineAPITime);
...
Firebug.NetMonitor.NetLimit = domplate(Firebug.Rep,
{
    ...
    limitTag:
        TR({class: "netRow netLimitRow", $collapsed: "$isCollapsed"},
            TD({class: "netCol netLimitCol", colspan: 9},
...
NetProgress.prototype =
{
    ...
    requestedFile: function requestedFile(request, time, win, xhr)
    {
        ...
        if (file)
        {
            ...
            file.appEngineCost = 0;
            file.appEngineTime = 0;
            file.appEngineCPUTime = 0;
            file.appEngineAPITime = 0;
...
function getHttpHeaders(request, file)
{
    try
    {
        ...
        try
        {
            file.appEngineCost = parseFloat(http.getResponseHeader("X-AppEngine-Estimated-CPM-US-Dollars").substr(1));
            var header = http.getResponseHeader("X-AppEngine-Resource-Usage").split(/(=| )/, 11);
            file.appEngineTime = parseInt(header[2]);
            file.appEngineCPUTime = parseInt(header[6]);
            file.appEngineAPITime = parseInt(header[10]);
        }
        catch (exc) {
            // ok - no header
        };
